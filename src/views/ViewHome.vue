<template>
  <div class="width">
    ...
    <div>
      <h1 class="mt-20 text-xl">You've spent a total of <span class="text-rb-blue">◎{{ totalSpend.toFixed(2) }}</span> on NFTs.</h1>
      <h1 class="my-10 text-xl">You've earned a total of <span class="text-rb-blue">◎{{ totalEarnings.toFixed(2) }}</span> from NFTs.</h1>
      <h1 class="my-10 text-xl">You've paperhanded a total of <span :class="neg(totalPaperhanded) ? 'text-rb-green' : 'text-rb-pink'">◎{{ totalPaperhanded.toFixed(2) }}</span>.</h1>
      <h1 class="my-10 text-xl">You're diamondhanding a total of <span :class="neg(totalDiamondhanded) ? 'text-rb-pink' : 'text-rb-green'">◎{{ totalDiamondhanded.toFixed(2) }}</span>.</h1>
      <h1 class="mb-20 text-xl">Your total {{ neg(totalProfit) ? 'loss' : 'profit' }} is:<span :class="neg(totalProfit) ? 'text-rb-pink' : 'text-rb-green'">◎{{ totalProfit.toFixed(2) }}</span>.</h1>
